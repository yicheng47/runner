// Mission history page. Live missions also appear in the sidebar tray, but
// stopped/completed missions need a stable place to be found and reopened.
...
export default function Missions() {
  ...
  const { active, past } = useMemo(() => {
    ...
    for (const mission of missions) {
      (mission.status === "running" ? active : past).push(mission);
    }
    return { active, past };
  }, [missions]);
          ...
          <header className="flex items-center justify-between gap-4">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm text-fg-2">
                Open live workspaces and review completed mission runs.
              </p>
            </div>
...
function MissionRow({
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="group flex cursor-pointer items-center justify-between gap-4 rounded-lg border border-line bg-panel p-4 text-left transition-colors hover:border-line-strong focus:outline-none focus-visible:border-fg-3"
    >
