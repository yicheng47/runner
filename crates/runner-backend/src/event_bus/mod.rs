...
/// Decouples the bus from the app event channel so the consumer thread can
/// be unit-tested with a recording fake. Production uses `ChannelBusEvents`.
/// Mirrors the `SessionEvents` trait in `session::manager`.
...
/// Production emitter — fans out to `event/appended`, `inbox/updated`,
/// `watermark/advanced` on the app event channel, from where the frontend
/// forwarder delivers them to the workspace UI.
pub struct ChannelBusEvents(pub EventChannel);

impl BusEmitter for ChannelBusEvents {
    fn appended(&self, ev: &AppendedEvent) {
        self.0.emit("event/appended", ev);
    }
    fn inbox_updated(&self, ev: &InboxUpdate) {
        self.0.emit("inbox/updated", ev);
    }
    fn watermark_advanced(&self, ev: &WatermarkUpdate) {
        self.0.emit("watermark/advanced", ev);
    }
}
