...
> Status: pre-alpha. Crew + runner config, mission start/stop, and PTY-backed sessions all run end-to-end on macOS / Linux. The orchestrator, the `runners` CLI, and the mission workspace UI are still being built — see `docs/logs/` for the latest progress snapshot.
