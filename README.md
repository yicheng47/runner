...
- **Themes** — Auto / Light / Dark chrome with an app palette and a terminal palette per mode (Runner and Catppuccin Mocha dark; Runner Light and Catppuccin Latte light; Runner Light, Runner Dark, Rosé Pine Dawn and Catppuccin Mocha for terminals, each mode picked in Appearance with a live preview), Inter bundled as the UI font, and JetBrains Mono (the Nerd Font Mono build) bundled for terminals.
