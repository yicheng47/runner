...
- **Themes** — Auto / Light / Dark chrome with two variants per side (Runner and Catppuccin Mocha dark; Runner Light and Catppuccin Latte light), terminal palettes that match the app by default or can be pinned (Runner Light, Runner Dark, Rosé Pine Dawn, Catppuccin Mocha), Inter bundled as the UI font, and JetBrains Mono (the Nerd Font Mono build) bundled for terminals.
