...
> Implementation plan: [`docs/impls/539-pi-runtime.md`](../impls/539-pi-runtime.md).
> Refreshed 2026-09-17. The 2026-09-10 draft predates hook status ([347](./archive/347-hook-based-session-status.md), [610](./610-windows-hook-status.md)), model discovery ([590](./archive/590-runtime-model-discovery.md)), provider marks ([593](./archive/593-provider-chat-icons.md)), chat permission posture ([596](./archive/596-chat-permission-posture.md)), the README feature matrix and the Copilot runtime ([540](./archive/540-copilot-cli-runtime.md)). Every claim below was re-probed on 2026-09-17 against the installed pi 0.85.1 on macOS (ten print-mode runs, two interactive PTY captures, and the package's `dist/` source), so this file supersedes the draft where they differ. Decided the same day with Jason: the system-prompt channel is the core of the runtime and ships in the first mission, not as a follow-up.
...
| Site | What changes for pi | Phase |
| --- | --- | --- |
| `assets.rs`, `chat_icon.rs` | A bundled pi mark with its tint, designed on the canvas first (phase 0); `pane_identity_icon`, `sidebar_tab_icon`, `command_palette.rs`, `settings/archived.rs`, `panes.rs`, `sidebar/tests.rs` and `roles/tests.rs` gain the row. *Exhaustive in `chat_icon.rs`.* | 1 |
...
| `ops/session.rs` `preferred_title` | No behaviour change: a role-backed chat keeps its handle under [#587](./archive/587-terminal-provided-titles.md)'s identity rule. The comment's "there is no system-prompt flag" aside gains the pi exception. | 1 |
...
- The provider mark, designed on the canvas first.
...
- Per-skill on/off for pi; honouring `PI_CODING_AGENT_DIR`, `PI_CODING_AGENT_SESSION_DIR`, `sessionDir` or a role's own `--session-dir` anywhere in Runner.
...
2. **Keys are caller-assigned; fresh and resume share `--session-id`.** Copilot's plan shape, no capture thread. A missing conversation keeps its key, because pi recreates the session under the same id. Print mode writes a "creating a new session with that id" warning on every fresh start; whether the TUI shows it too is for the fixture to record. The probe assumes pi's default session directory; a role that overrides it (`--session-dir`, `PI_CODING_AGENT_SESSION_DIR`, `sessionDir`) makes every relaunch look fresh, which re-sends the lead's goal onto an existing conversation. Documented, not handled, in v1.
3. **Native fork is one direct spawn.** `--fork <source> --session-id <new>` needs no headless step, so pi forks to a pane or tab exactly like claude-code under [60](./archive/60-fork-chat-to-pane-or-tab.md). The forked session gets its own system-prompt file, composed from the same rows.
...
6. **Mission slots pass `--approve`; chats pass nothing.** An unattended slot in a directory with `.agents/skills` or `.pi` resources would sit on "Trust project folder?" with no status signal. `--approve` is pi's documented one-run `trustOverride`: it trusts for that run only and writes nothing to `~/.pi/agent/trust.json`. Not taken: answering `project_trust` from the status extension, which the probe shows would work; it would tie trust to the phase-2 bridge and to `ctx.mode`, while `--approve` is one argv token in phase 1. A chat has a human at the prompt, whose answer pi saves as usual, in line with [596](./archive/596-chat-permission-posture.md). Accepted risk: trusting loads the project's `.pi` extensions and settings, the same boundary the crew's own bash tool already has in that directory, since pi has no sandbox. For Jason's sign-off.
...
10. **Rekey through the Claude drop file.** `/new`, `/resume` and `/fork` inside a pane replace pi's session, and `agent_session_key` would go stale, so the next relaunch would reopen the old conversation. On every `session_start` whose `getSessionId()` differs from `RUNNER_PI_SESSION_KEY`, the extension writes `{"session_id": "<new id>"}` atomically (temp file, rename) to `RUNNER_PI_REKEY_PATH`, which Runner sets to `claude_rekey::drop_path(app data, runner session id)`. `ClaudeSessionKeyWatcher` already runs for every runtime, checks the id is a UUID (pi's are v7), rekeys the row and emits the update. No new Rust beyond three env vars.

## Implementation Phases

### Phase 0 — design

The pi mark beside the Claude, Codex, Trae and Copilot marks on `design/runner.pen`, at 12/14/16/24 px, in the rail on both themes, with a tint Jason picks. The role form without a permission control reuses existing components.
