...
Tracking issue: [#437](https://github.com/yicheng47/runner/issues/437), closed. Shipped in [Runner 0.8.0](https://github.com/yicheng47/runner/releases/tag/v0.8.0) on 2026-09-06; archived with the [implementation record](../../impls/archive/windows-nightly/README.md). macOS and Windows now develop together on `main`. Current development instructions and remaining follow-ups live in [Windows development](../../arch/windows.md). The original scope and dated decisions below are retained as history.

## Motivation

Main integration follow-up, 2026-09-06: Jason reports Windows testing and most macOS regression checks passed and requested merging the port through [#489](https://github.com/yicheng47/runner/pull/489). Development and nightlies move to `main`; `release.yml` also builds an unsigned Windows installer alongside the Mac DMG into the same manually published draft. Windows production builds check stable releases, while nightlies retain `nightly-win`. This supersedes this document's original production-channel exclusion; signing remains outstanding.

September 6 scope clarification: Windows includes update detection, the sidebar download indicator, and manual-download controls. Jason has added **Phase 4 — Windows installer and upgrades** as remaining port work: a per-user Inno Setup EXE installer, Windows signing, and user-run upgrades that preserve settings and missions. Automatic update download/installation and a production channel remain outside scope. The crew mission smoke test is accepted. See the [remaining phase](../../impls/archive/windows-nightly/plan.md#phase-4--windows-installer-and-upgrades-remaining) and [program decisions](../../impls/archive/windows-nightly/plan.md#decisions).
