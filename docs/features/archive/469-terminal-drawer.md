...
**Direction change (2026-09-04):** drawer shells now relaunch lazily. Quit still stamps live drawer shells with launch claims, but startup leaves those claims pending; the active shell resumes when its open drawer first becomes visible, and other chips resume when selected. Hidden drawers spawn nothing until opened, while shells that exited on their own have no claim and remain on the Shell exited card. Pane shells in terminal-only tabs still relaunch eagerly under the 64 rule.
...
- **Relaunch** restores chip order, the active chip, the open state, and the height immediately, but defers each fresh shell process until that chip is visible. Startup excludes drawer shell ids from the eager launch-claim consumer; opening or activating the drawer atomically takes the active chip's pending claim and resumes it at the drawer's real size. A missing cwd resolves to the nearest existing ancestor, then the project, then `$HOME`, with the substitution written into the terminal — the 64 rule.
...
2. **Sessions and relaunch.** Spawn through `session_start_shell` with the resolved cwd; close through `session_close`; on launch, restore the tab's drawer state and lazily resume a claimed shell the first time its chip is visible.
