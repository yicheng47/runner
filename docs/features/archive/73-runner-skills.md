...
Tracking: [#73](https://github.com/yicheng47/runner/issues/73). Status: closed 2026-09-13 — M1 (the Settings → Skills pane) shipped in [#531](https://github.com/yicheng47/runner/pull/531) and the MCP catalog in [#555](https://github.com/yicheng47/runner/issues/555); M2 and M3, the per-runner allowlists, continue as [#577](https://github.com/yicheng47/runner/issues/577). Implementation program: [`docs/impls/archive/local-skills/`](../../impls/archive/local-skills/README.md) (M1 view skills by runtime → M2 allowlist backend → M3 allowlist app → later).
...
### What Codex provides (0.153.4, verified 2026-09-09)

| primitive | scope | what it does |
|---|---|---|
| `~/.agents/skills/<name>/SKILL.md` | user | the documented user scope. `$CODEX_HOME/skills` (`~/.codex/skills`) is the older location and is **still read** — a headless `codex exec "list every skill"` on this machine returned all 11 names under `~/.codex/skills` and all 28 under `~/.agents/skills`. Runner scans both. |
| `<repo>/.agents/skills`, `/etc/codex/skills`, `$CODEX_HOME/skills/.system` | project, admin, system | the project's, the machine's, and Codex's own (`imagegen`, `skill-creator`, …). Not catalogued, always load — the codex counterpart of Claude Code's project and bundled skills. |
| `[[skills.config]]` in `~/.codex/config.toml`: `path = "<…/SKILL.md>"`, `enabled = false` | global | hides the skill from every new Codex session. Path-keyed, not name-keyed. The Codex desktop app's skill sheet toggles the same setting. Takes effect on the next session. |
| `agents/openai.yaml` beside `SKILL.md`: `policy.allow_implicit_invocation: false` | per skill | Codex's invocation policy — the model may not start the skill on its own; the user invokes it explicitly. Codex's counterpart of Claude Code's `disable-model-invocation`; Codex ignores that frontmatter key and has no counterpart of `user-invocable`. |
| `-c skills.config=[…]` | per launch | Codex's generic config override on the CLI. Whether it carries a per-session `enabled = false` list is unverified — M2 tests it before deciding the codex allowlist route. |
...
- **codex**: two roots, both scanned and shown in one case-insensitive alphabetical list (not grouped by root; Jason, 2026-09-09) — `~/.agents/skills/<name>/SKILL.md` (user scope) and `$CODEX_HOME/skills/<name>/SKILL.md` (`~/.codex/skills`, legacy, still read). Frontmatter `name` and `description` only — `manual` comes from `agents/openai.yaml` → `policy.allow_implicit_invocation: false` (absent file or key → not manual), and `hidden` / `name-only` never apply. A name present under both roots is listed twice, adjacent, as Codex itself does. Global state from `~/.codex/config.toml` → the `[[skills.config]]` entry whose `path` equals the entry's marker file as scanned (root-joined, not canonicalized): `enabled = false` = off; absent or `true` = on.
...
The Skills pane toggles a skill on or off **for every session of that runtime, inside Runner or not**. For claude-code, by editing `skillOverrides` in `~/.claude/settings.json`: off → set `"off"`; on → delete the entry (Claude Code's default is on; a `"name-only"` / `"user-invocable-only"` entry is also cleared by toggling on, and the pane says so on hover). `ops::skills::set_global_enabled(runtime, name, enabled)` does a structured read-modify-write like `ops/mcp.rs` does for `mcp_servers.runner`: parse, touch one key, write back; a file that does not exist is created with only that key; a file that fails to parse is left alone and the toggle reports the error. Running sessions are unaffected until their next launch — the pane says so in its caption. For codex, by editing `[[skills.config]]` in `~/.codex/config.toml` through `toml_edit` (the crate `ops/mcp.rs` already uses for that file): off upserts one entry `{ path = "<marker path as scanned>", enabled = false }`; on removes every entry whose `path` matches; other tables and entries are untouched; a missing file is created with only that array; a file that fails to parse is left alone and the toggle reports the error.
...
Designed 2026-09-09 in `design/runner.pen`: `Settings — Skills` (`s8mYyG`, Claude Code), `· Codex` (`g8KuD3`), `· skill detail` (`cwayt`), `· skill edit` (`peYFQ`), `· empty root` (`skWOh`), `· missing root` (`GIyYH`). **This pane is the first milestone (M1) and ships alone.** One runtime at a time, not one card per runtime: 39 personal skills per runtime stack too tall, and the row rules differ per runtime.

- **Title row**: `Skills`, a **Refresh** button on the right (rescans every runtime).
- **Controls row**: a runtime **dropdown** on the left (`StyledSelect` grammar, `Claude Code ▾`; options are the runtimes that have a catalog, so a new runtime is one more option), a search field over name + description filling the rest.
- **Meta line** under the controls: the scanned directory as a faint mono path (codex shows both, `~/.agents/skills · ~/.codex/skills`), then `39 skills · 2 off`. Below it a one-paragraph faint caption. Claude Code: *"Toggles hide a skill from every new Claude Code session, inside Runner or not; the only write is the skillOverrides key in ~/.claude/settings.json. Click a row to read a skill, hover it to edit. Bundled skills (code-review, loop, …) and project skills always load and are not listed."* Codex: *"Toggles hide a skill from every new Codex session, inside Runner or not; the only write is a [[skills.config]] entry in ~/.codex/config.toml. Click a row to read a skill, hover it to edit. Codex's system skills (~/.codex/skills/.system) and plugin skills always load and are not listed."*
- **List card**: one panel, hairline-separated **two-line rows** (~58 px): line one the skill name (folder name as the model sees it, never title-cased) with its flag badges beside it — `manual` (claude-code: `disable-model-invocation`; codex: `agents/openai.yaml` `policy.allow_implicit_invocation: false`), `hidden` (`user-invocable: false`, claude-code only), `symlink` (target on hover), `name-only` / `user-invocable-only` (the `skillOverrides` value, claude-code only), `problem` in danger tone with the parse error on hover — `symlink` and `problem` are Runner's own, the rest belong to the runtime that reads them and never show on the other's rows; line two the description's first sentence, faint, one line, ellipsised. Right edge on every row: the `Toggle` widget (`ui/toggle.rs`, the shipped 32 × 18 accent-at-15 % track, not a solid accent bar) bound to the skill's global state; off rows render at reduced opacity. On hover one action appears left of the toggle: an edit glyph that opens the skill's detail modal in edit mode. No Reveal or Copy on the row.
- **Empty / missing root**: the list card holds one faint row — *"No skills in ~/.agents/skills or ~/.codex/skills."* or *"… (neither directory exists)."*, single-root wording for claude-code — and the meta line reads `0 skills`.
- **Detail modal** (`cwayt`): a row click opens a centered 680 × 790 modal in the Start Mission modal's chrome (scrim, shadow, header band with ✕). Header: name, badges, an **Edit** button. Then the full description, the path row (path in mono, symlink target after `→`; a quiet `Reveal in Finder` on the right, `Reveal in Explorer` on Windows), the folder's top-level entries as chips, and the **Enabled in Claude Code** / **Enabled in Codex** row: label, a two-line note saying what it writes, the same toggle as the row. Below, a **document panel** that fills the remaining height and scrolls on its own, with a bar reading `SKILL.md · 37 lines` and a `Preview | Source` switch: Preview renders the frontmatter as a key/value table above the body rendered through `surfaces/mission_markdown.rs` (never raw `---` lines); Source is the file text byte for byte in monospace. Rendered from the file on open, not cached. Esc, ✕ or a backdrop click closes.
- **Edit mode** (`peYFQ`): Edit swaps the document panel for the multi-line `TextField` (`TextFieldKind::Textarea`, monospace) holding the whole file, frontmatter fences included; the header gains an `editing` chip and drops Edit and ✕; a footer appears with a faint hint (*"Writes SKILL.md in place — through the symlink into …"*), **Cancel** and an accent **Save**. ⌘S saves. Esc, Cancel or a backdrop click go one level back — to Preview, not out of the modal — and ask first when the buffer is dirty (Jason, 2026-09-09, from the smoke test); a second Esc from Preview closes. Save writes the file in place (a symlinked skill writes its target), re-parses that skill, and refreshes the modal and the row. It is a plain-text editor, not a Markdown editor: no highlighting, no toolbar.
- No create, no delete, no install. The shell's file-link editor setting (feature 458) is not used by this pane; the modal is the editor.
...
- Codex per-launch visibility control in v1. `-c skills.config=…` is the candidate and M2 verifies it; the managed `CODEX_HOME` mirror (symlinked `auth.json`, copied `config.toml`, a curated `skills/`) stays declined as in feature 52.
...
0. **Design** — `design/runner.pen`: the Skills pane, detail modal and edit mode landed 2026-09-09 (node ids in §Surfaces); the runner-form Skills field with its checklist popup and chip summary, and the detail-page block, before M3. Reuses the Agents card and the model-field popup grammar; the only new element is the checklist row.
1. **M1 — View, switch and edit skills by runtime** — `skills.rs` catalog over each runtime's global directory (frontmatter parsing, flags, problems, file list, global state) + the Settings → Skills pane as described above, including the detail modal with its editor (`save_skill`) and the claude-code global toggle (`set_global_enabled`). No schema, no spawn change: nothing about how Runner spawns moves; the toggle changes what Claude Code loads everywhere, which is its point. Ships alone.
...
- `ops::skills::save_skill`: writes the given text to the skill's `SKILL.md` and nothing else, follows a symlinked skill folder to its target, refuses a name that is not in the catalog, returns the re-scanned entry; a write error is returned, never a panic.
...
- `ops::skills::set_global_enabled` (codex): off upserts one `[[skills.config]]` entry with the scanned marker path, on removes every matching entry, other tables (`model`, `[projects.*]`, `[mcp_servers.*]`, `[tui]`) and unrelated entries survive, a missing file is created with only the array, an unparsable file is left untouched and reported.
...
- Manual (M1, codex): toggle `books` off → `~/.codex/config.toml` gains exactly one `[[skills.config]]` entry (`path` = `~/.codex/skills/books/SKILL.md`, `enabled = false`) and nothing else changes; `codex exec "list every skill you can see"` no longer names `books`; toggle on → the entry is gone; a `lark-*` row (under `~/.agents/skills`) toggles the same way.
- Manual (M1, editor): open `daily-brief`, Preview shows the frontmatter table and the rendered body, Source shows the file verbatim; Edit → change one word → Save → the file on disk (through the symlink) carries the change and nothing else (`git diff` in the memory repo), the row's description updates if it was the description; Edit → change → Esc asks before discarding; Cancel leaves the file untouched.
