...
- **Light terminal palette** (`LightTerminalTheme`, JSON `lightTerminalTheme`): **Rosé Pine Dawn** (`rose-pine-dawn`, the default; Jason after the smoke test: "this looks much better than our previous own Runner light theme"), **Runner Light** (`runner-light`), **Runner Dark** (`runner-dark`), **Catppuccin Mocha** (`catppuccin-mocha`). Known consequence of the default: Dawn's warm cream ground (`#FAF4ED`) sits on Runner Light's cool `#F6F6F8`, so a terminal pane reads as a faint warm rectangle against the chat until the app's light variant is warmed to match.
- **Dark terminal palette** (`DarkTerminalTheme`, JSON `darkTerminalTheme`): **Runner Dark** (`runner-dark`, the default; today's `Runner` palette under its honest name), **Catppuccin Mocha**, **Rosé Pine Dawn**, **Runner Light** — the two light palettes are there for a light terminal inside a dark app.

**Runner Light** (`palette::RUNNER_LIGHT`) is the palette designed on the same canvas as the app tokens (`Runner Light · terminal palette`, `W7oJm`): 16 ANSI colors plus fg, bg, cursor, selection, with the app `bg` as its ground so a terminal pane and the chat around it are one surface, as they are in Carbon. **Rosé Pine Dawn** (`palette::ROSE_PINE_DAWN`, added 2026-09-11 at Jason's ask) is the official Rosé Pine Dawn terminal palette, canvas frame `Rosé Pine Dawn — terminal palette (529)` (`VLI02`). Both live beside the other palettes in `crates/runner-terminal/src/palette.rs`. Monokai was removed 2026-09-11 for its licence.
...
- Settings → Appearance: the Light card's Terminal palette reads Rosé Pine Dawn, Runner Light, Runner Dark, Catppuccin Mocha and the Dark card's Runner Dark, Catppuccin Mocha, Rosé Pine Dawn, Runner Light, with the defaults selected on a fresh profile and on a profile whose file said `terminalTheme: "runner"`, `match-app` or `monokai`; picking Runner Light or Rosé Pine Dawn as the dark pick gives a light terminal in a dark app, picking Runner Dark or Mocha as the light pick gives a dark terminal in a light app; both survive a restart. Settings → Terminal has no Theme row.
