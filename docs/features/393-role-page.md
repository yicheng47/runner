...
- **Narrow windows**: Runner has no enforced minimum window size; the smallest it restores is 640 × 480, which with the default 240 px sidebar leaves a 400 px page and a 336 px table. The role page wraps the prompt card under the profile column once the two no longer fit side by side (below about 750 px of page), and the table keeps its Role column at 176 px or more by dropping columns as it narrows: Crews first, then Effort, Model, Runtime and Last active. At the 640 px floor only Role and the row actions remain; long names, handles, commands and directories truncate throughout, and nothing scrolls sideways.
...
- On macOS and Windows, both pages fit the 640 px minimum with no horizontal scroll, the role page stacked and the table narrowed as Narrow windows describes.
