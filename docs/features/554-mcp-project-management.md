...
- **`project_delete(id, force = false)`.** Follows the sidebar's semantics: member missions archive, member terminals close, member chats archive, and the project row and node go in one transaction. The sidebar shows a confirm dialog before it kills anything; an MCP caller sees no dialog, so the tool refuses while anything in the project is still live and names it in the error, unless `force` is true, in which case it takes the sidebar's path and kills them. "Live" covers both kinds of member: a chat or terminal session in a member tab whose row is `Running`, and a member mission whose status is `Running` (mission sessions hang off the mission, not a tab node, so `container_children.session_ids` alone would miss them). Returns the archived mission ids and chat session ids; `ops::project::project_delete` currently returns `()` and only its `_impl` returns the chat ids, so the wrapper grows to return both while keeping its event fanout.
- **`mission_set_project(mission_id, project_id | null)`.** Files a mission into a project or unfiles it (`null`). Resolves the mission's node through `repo::node::find_by_ref(NodeType::Mission, …)` and the target project's node, appends the mission at the end of the target container, and goes through `ops::node::node_move` so the mission's `project_id` pointer and the layout events are rewritten the way a sidebar drag rewrites them. `node_move` demands the complete order of the destination's unpinned children (`repo::node::move_and_reorder` rejects anything else); the sidebar builds that list in `sidebar_logic::complete_unpinned_scope_order`, which lives in the app crate, so the backend gets a small equivalent: the destination's current unpinned children in position order, minus the moved node, plus the moved node at the end. A pinned mission keeps its `pinned_position` and only its parent changes, so it is left out of that order. A mission already in the target container returns unchanged. Moving a mission does not change its `cwd`; that was copied at start.
...
- `ops::project`: a `live_members(conn, project_id)` helper over `ops::node::container_children` returning the running tab session ids (via `repo::session::get_row` status) and the running mission ids (via the `MissionStatus` already in `ContainerChildren`), used by the delete guard and returned in its error; `project_delete` returns `{ archived_mission_ids, archived_session_ids }` instead of `()` (its one caller, the sidebar, ignores the value).
- `ops::node`: an `append_order(conn, parent_id, moved_id) -> Vec<String>` helper that produces the `ordered_ids` `node_move` requires, used by `mission_set_project`.
- `crates/runner-backend/src/mcp/server.rs`: add the four names to the registry test.
- Tests in `mcp/tools/project.rs` and `mcp/tools/mission.rs` against `db::open_in_memory()`: create rejects a relative or missing cwd and appends position; rename returns the new name; delete refuses with the running ids while a member session row or a member mission is `Running`, succeeds with `force`, and archives an unfiled-after-delete mission; `mission_set_project` moves a mission in and back out, lands at the end of the destination order, keeps a pinned mission's `pinned_position`, and leaves `cwd` alone.
...
- [ ] `project_delete` on a project with a running chat or a running mission returns the running ids and does nothing; with `force: true` they are killed and archived and the project is gone; on a project with only stopped members it archives them and returns their ids.
