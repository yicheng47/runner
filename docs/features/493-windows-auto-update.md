...
3. The installer's `PrepareToInstall` waits up to 30 seconds for `/WAITPID` to exit before its existing in-use check, installs, and a plain `[Run]` entry gated on `/RELAUNCH=1` (no `postinstall`, no `skipifsilent`) starts the new `Runner.exe` and lets Setup exit.

Failures leave the current installation usable. If a sidecar is still held open by a process outside Runner (an external agent with `runner-mcp.exe` running), the existing in-use check refuses, Inno Setup rolls back nothing because nothing was copied, the message box explains it, and `DeinitializeSetup` relaunches the old `{app}\Runner.exe` because `/RELAUNCH=1` was passed and the install did not complete. If file copying fails midway, Inno Setup aborts and rolls back the copy, and the same relaunch applies. On relaunch the updater finds the staged installer still newer than the running app, re-verifies it, and the dialog shows the Failed state with **Install and restart** again and the log path in Settings → Diagnostics.
...
- [ ] An external process holding `runner-mcp.exe` open: the installer refuses, the old Runner relaunches, and the dialog shows the Failed state with the log path in Diagnostics.
- [ ] `test-installer.ps1` covers `/WAITPID`, `/RELAUNCH`, and relaunch after refusal; Windows CI, the macOS updater checks, and `make verify` pass.
