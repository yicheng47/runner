...
- [469 — Terminal drawer](./469-terminal-drawer.md) — inside a chat tab the terminal becomes a drawer under the pane split (⌘J / a `panel-bottom` header toggle, drag-resized, one or more shell chips with `+`), hide keeps shells running and a chip's × kills; the header's trailing group becomes surface toggles only and Fork moves beside Stop; the empty pane offers New chat with a ⌘N hint; the sidebar's terminal-only tab is untouched; supersedes 64's "terminal is a pane" for in-tab terminals ([#469](https://github.com/yicheng47/runner/issues/469)).
