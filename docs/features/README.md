...
- [475 — Codex updates: Runner owns them, the TUI never asks](./475-codex-auto-update.md) — codex 0.153.0's `check_for_update_on_startup` is passed as `-c …=false` on every Runner-built codex argv so sessions never stall on the TUI's update prompt; Runner learns installed and latest versions in runtime discovery, shows them in Settings → Agents, and runs `npm install -g @openai/codex@latest` itself under a default-on **Update codex automatically** switch; no grid scraping, no restart dance ([#475](https://github.com/yicheng47/runner/issues/475)).
- [437 — Windows nightly](./437-windows-nightly.md) — a published `nightly-windows` pre-release zip anyone can download (the macOS `nightly` stays a draft); measured gap on `main` 2026-09-04 is ~17 compile errors and three behavioural areas — Job Object process lifecycle, agent spawning (`PATHEXT`, `;` PATH, `runner.cmd` shim, `USERPROFILE`), and a named-pipe MCP transport — plus a `windows-latest` CI job; native spawn, Ctrl keymap, no installer/signing/updater ([#437](https://github.com/yicheng47/runner/issues/437), reopened from the 2026-08-27 won't-do).
