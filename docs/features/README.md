...
- [542 — Stop, resume, and restart a single mission slot](./542-slot-restart.md) — per-slot Stop / Resume / Restart on the rail card in the header's tinted icon style, Restart as an immediate kill-plus-fresh-respawn of the same row that re-delivers the cold-start first turn on argv (launch prompt for the lead, preamble plus brief for a worker) and posts a `slot_restarted` signal plus a note to the lead; every fresh-fallback resume gets the same first turn; a per-slot stopped card replaces "Mission paused" while other slots run, and the header Stop confirms "Stop all N slots?" ([#542](https://github.com/yicheng47/runner/issues/542), bug, P1).
- [541 — Bypass mode never waits on a first-run dialog](./541-bypass-never-prompts.md) — since #527 made Bypass the mission default, claude-code's first-use consent dialog parks a slot nobody is watching; Runner adds `skipDangerousModePermissionPrompt` to the `--settings` JSON it already passes when the effective mode is Bypass, scoped to the spawn with no write to the user's config, the runner-form copy stops promising a dialog, and the trust dialog under bypass gets checked in a fresh cwd ([#541](https://github.com/yicheng47/runner/issues/541), bug, P1).
