...
- [65 — Runtime default model and effort](./65-runtime-default-model-effort.md) — read-only surfacing of each CLI's own configured model/effort (`~/.codex/config.toml`, `~/.trae/traecli.toml`, `~/.claude/settings.json`, `~/.qoder/settings.json`) as a caption on Settings → Agents rows and as the runner form's `default (…)` placeholders; Runner never writes those files ([#380](https://github.com/yicheng47/runner/issues/380)).
