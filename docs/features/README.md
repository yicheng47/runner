...
- [40 — Projects](./40-projects.md) — cwd-bound project containers grouping chats and missions in a Codex-style PROJECTS sidebar section; a `projects` table plus `project_id` on sessions and missions, "Start from scratch" / "Use an existing folder" creation, and cwd inheritance for work started inside a project.
