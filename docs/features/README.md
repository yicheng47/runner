...
- [469 — Terminal drawer](./469-terminal-drawer.md) — inside a chat tab the terminal becomes a drawer under the pane split (⌥F12 / a `panel-bottom` header toggle, drag-resized, one or more shell chips with `+`), hide keeps shells running and a chip's × kills; the header's trailing group becomes surface toggles only and Fork moves beside Stop; the empty pane offers New chat with a ⌘N hint; the sidebar's terminal-only tab is untouched; supersedes 64's "terminal is a pane" for in-tab terminals ([#469](https://github.com/yicheng47/runner/issues/469)).
- [474 — Mission terminal drawer](./474-mission-terminal-drawer.md) — one mission-wide terminal drawer beneath the central Feed / runner-tab surface, preserving the same shells while the user moves across mission tabs; it reuses 469's drawer controls and `shell` runtime without turning the shell into a crew slot or mission participant ([#474](https://github.com/yicheng47/runner/issues/474)).
