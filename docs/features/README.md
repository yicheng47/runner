...
- [466 — Sessions outlive the app process](./466-sessions-outlive-the-app.md) — session PTYs move out of the app process into a small background host, so quitting, updating, or crashing Runner.app no longer stops running agents; on relaunch sessions reattach with screen and recent scrollback restored, paused HITL/router delivery reconciles without firing twice, and quit becomes an explicit leave-running vs stop-everything choice ([#466](https://github.com/yicheng47/runner/issues/466)).
