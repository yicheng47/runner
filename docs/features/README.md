...
- [606 — Rail glyph liveness and a folder that opens](./606-rail-glyph-liveness.md) — one rule for every sidebar glyph: hue is identity and only provider marks have one, everything else is neutral and dims with opacity when no session is running, the accent stays at the tail for state; plus a closed/open folder pair selected by the project's expansion state ([#606](https://github.com/yicheng47/runner/issues/606), P2).
- [604 — Rename the runner entity to role](./604-rename-runner-to-role.md) — "Runner" names the product, the reusable agent configuration and the CLI at once, and the Start a chat modal shows all three plus a stray "persona"; the entity becomes `role` (`runners` → `roles`, `slots.role_id`, `runner_*` MCP tools → `role_*`, `surfaces/runners/` → `surfaces/roles/`, "Runner default" → "Role default"), while the product name, bundle id, repo, the CLI's `runner signal` / `runner msg` verbs, and "worker" as a live non-lead mission participant all stay put ([#604](https://github.com/yicheng47/runner/issues/604), P1).
- [596 — Chats inherit the agent's own permission default](./596-chat-permission-posture.md) — chat spawns stop applying the runner row's permission flags so each agent runs on its own default, leaving `MissionPermissionMode` the single place a posture is decided; also fixes `permission_mode_args(Trae, Auto)`, which emits a `--permission-mode auto` that TRAE CLI rejects and which has crashed every Trae chat ever started ([#596](https://github.com/yicheng47/runner/issues/596), P1).
- [593 — Provider icons for chats](./593-provider-chat-icons.md) — Claude's starburst, the OpenAI knot and the Trae screen across chat identity surfaces, with bundled assets, fixed brand tints and dimming for stopped chats; references Orca, cmux and lobe-icons ([#593](https://github.com/yicheng47/runner/issues/593), P2).
- [610 — Hook-based session status on Windows, plus the 347 follow-ups](./610-windows-hook-status.md) — lift the per-runtime Windows gate on the Claude Code, Codex and GitHub Copilot CLI status bridges after a JASONPC smoke of hook execution, the reporter, feed paths and cleanup; then the deferred status details and the title-heuristic removal where hooks are validated ([#610](https://github.com/yicheng47/runner/issues/610), P1).
