...
- [557 — Translucent window backdrop](./557-window-backdrop.md) — Ghostty-style blurred backdrop on GPUI's built-in `WindowBackgroundAppearance::Blurred`: an Appearance → Window backdrop row (Solid default, Translucent with a 70–95% opacity stepper), applied live to every window, with exactly four ground surfaces carrying the alpha (app root, sidebar, pane chrome, the terminal's base fill) while overlays, dialogs, and cells with their own background stay opaque; Liquid Glass chrome, blur-radius control, per-surface opacity, and background images stay out, Windows 11 Mica is a follow-up on `nightly-windows` ([#557](https://github.com/yicheng47/runner/issues/557)).
- [555 — Settings → MCP, one catalog of MCP servers for every agent](./555-mcp-settings.md) — the catalog #73 deferred and #530 reserved the nav name for: a pane that reads the agents' global MCP configs (`~/.claude.json`, `~/.codex/config.toml`, `~/.trae/traecli.toml`) as one list keyed by name with no Runner-side store, a chip per agent that registers or removes a server in that agent's config, an add/edit modal (stdio or http, agents to write into) that translates one definition into each format through the read-modify-write `ops/mcp.rs` already does, conflicts flagged per row, and Runner's own server as the built-in first card so the `Runner MCP` line leaves the Agents cards; project-scoped servers, per-runner picks, enable-without-unregister, and auth flows stay out ([#555](https://github.com/yicheng47/runner/issues/555)).
