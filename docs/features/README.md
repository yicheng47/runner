...
- [505 — macOS nightly replaces Runner](./505-macos-nightly-replaces-runner.md) — the nightly bundle becomes `com.wycstudios.runner` / `Runner.app` with only the baked Sparkle feed differing, so installing a nightly switches the install to the nightly channel like Windows; bundled with #504 ([#505](https://github.com/yicheng47/runner/issues/505)).
- [504 — One nightly release for both platforms](./504-single-nightly-release.md) — `nightly` carries the DMG, appcast, installer, and signature; the Windows updater reads `nightly`; installed nightlies hop once by hand and `nightly-win` is then deleted; bundled with #505 ([#504](https://github.com/yicheng47/runner/issues/504)).
- [502 — One nightly for both platforms](./502-unified-nightly.md) — one `nightly.yml` dispatch builds macOS and Windows from one stamp by default with a `platform` input for single-platform cuts; the macOS `nightly` release becomes a public prerelease with a signed appcast so Mac nightlies self-update like Windows ones; nightlies are identified by commit independently of official version tags ([#502](https://github.com/yicheng47/runner/issues/502)).
