...
- [554 — Manage projects over MCP](./554-mcp-project-management.md) — the write half of the `project_*` MCP surface: `project_create` (absolute, existing `cwd` checked at the boundary), `project_rename`, `project_delete` (sidebar semantics, but refuses while a member session runs unless `force`), and `mission_set_project` to file or unfile a mission through the node tree; reordering, tab moves, cwd changes, and a CLI command stay out ([#554](https://github.com/yicheng47/runner/issues/554)).
