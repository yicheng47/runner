...
- [510 — Remote SSH session](./archive/510-remote-ssh-session.md) — closed as not planned ([#510](https://github.com/yicheng47/runner/issues/510), 2026-09-17): superseded by the session host ([#645](https://github.com/yicheng47/runner/issues/645)). An `ssh -t exec` chat leaves hook status, Windows quoting, PATH and env, sidecar version skew and agent detection unsolved for the mission-slot case; a host on the remote machine reached over the same ssh solves them all, so remote chats and slots ship together under #645 phase 2. Spec kept as the record of the host field and the local-only checks.
