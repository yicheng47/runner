...
- **Entry points**: the empty-pane placeholder gains **New terminal** beside **New chat**; `⌘T` opens a terminal in the active tab — into the focused empty pane when there is one, otherwise splitting the focused pane. No modal: a terminal has nothing to configure that `cd` does not cover.
...
- **A terminal is a pane, not a tab type.** No new `NodeType`: a terminal is a direct session row whose `runtime == "shell"`, sitting in a pane of the tab's existing layout. The sidebar, pinning, reorder, drag-drop and `⌘1`–`9` never learn a third case. Shell rows must always be placed into a pane on creation, because the node reconciler wraps any uncovered direct session in its own single-pane tab — an unplaced shell would sprout a sidebar row.
