...
- The sidebar mission flag is green while any slot is live, and a mission opens on the runner sessions rail regardless of the last rail toggle.
- Feed rows: signal and warning rows share the message rows' avatar column, warnings and asks show their text without a toggle, and the ask card's prompt is body size.
...
- [ ] Header Stop opens the confirm and names the count; per-card Restart opens its confirm; per-card Stop and Resume act immediately.
