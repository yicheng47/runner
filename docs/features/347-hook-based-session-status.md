...
> Review correction, 2026-09-14: Claude Code 2.1.270 emits `idle_prompt` after a user-tunable idle delay, not immediately at turn completion. Jason changed slice 1 to use `Stop` and `StopFailure` for Idle and retain a real `Notification(idle_prompt)` as secondary confirmation. Continuation can briefly overturn the dot; Busy/Idle gates no behavior. The [slice 1 brief](../impls/347-hook-bridge.md) records the revised contract. TRAE's documented immediate idle notification still needs binary verification.
...
| Ready | Main-agent `Stop`, or `StopFailure` where the runtime has one, corrected by the next work event if a hook continued the turn. | Display only — it gates nothing, so a Ready that a continuation overturns is a flicker rather than a delivery bug. No runtime offers a continuation-proof boundary; this is why that is acceptable rather than a compromise. No new outcome text in the tooltip yet. |
...
2. Port the bridge mechanism, exercise Claude's turn-end and continuation sequence, and verify TRAE's documented immediate `idle_prompt` in its binary before enabling that adapter. The Claude review established that its idle notification is delayed and user-disableable. Claude and Codex turn-end displays accept the temporary error from a continued `Stop`; delivery remains independent of Ready.
