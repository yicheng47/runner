...
Codex therefore needs two explicit concessions rather than silent guesses. Its `Stop` is provisional: treat it as Ready for display, but hold automatic inbox delivery for a short settle interval and withdraw it if `UserPromptSubmit` or a tool event follows, and label the source so the stricter delivery gate can tell a provisional boundary from a confirmed one. Its `PermissionRequest` needs the hold-and-cancel interval described in the coverage table, because nothing else separates a shown prompt from one a hook answered. Both are heuristics, both are confined to Codex, and neither may be described in the UI as a confirmed state.

The Codex trust gate is a product requirement, not a detail. Runner cannot install a working Codex hook silently: the user has to review and trust it through `/hooks`, and `--dangerously-bypass-hook-trust` is not an acceptable workaround. Phase 1 must show Status unavailable on a Codex session whose hook is untrusted, and say plainly what to do about it.
...
| Codex | Working, a debounced `Approval needed`, and a provisional Ready that holds inbox delivery for a settle interval — and no `Answer needed` at all, because no event exists. A Codex session whose hook has not been trusted through `/hooks` shows Status unavailable with an explanation, never a guess. This is the runtime that constrains the feature. |
...
2. Run a capability spike against the installed CLIs — a throwaway hook script, not production code. Drive a real approval prompt, an auto-approved one, a question, a plan approval, a completed turn, an interrupt and a failed turn on Claude Code and Codex, TRAE CLI is already checked and can be the first adapter rather than the last. Record which events fire, in what order, and how long an auto-approval takes to resolve. The result fixes the phase-1 vocabulary per runtime and the amber hold interval, and it is what the implementation plan in `docs/impls/` is written from.
