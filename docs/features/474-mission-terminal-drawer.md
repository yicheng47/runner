...
- Normal app relaunch restores the drawer state without eagerly spawning its shells. The launch claim marks shells that were live at quit; the active shell resumes the first time the drawer is visible, and the remaining shells resume when their chips are selected. A shell that exited without a claim stays exited. This feature does not broaden the separate direct-shell recovery policy after an abrupt app termination.
...
2. **Shell lifecycle.** Start shells at the mission cwd, attach one terminal bundle per drawer shell, close with foreground-process confirmation, restart exited shells in place, lazily consume relaunch claims when a drawer shell first becomes visible, and close all drawer shells on mission archive/delete.
