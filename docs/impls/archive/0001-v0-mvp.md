...
>
> Companion to `docs/arch/arch.md` (architecture) and `docs/product/vision.md` (scope). This file is the stable MVP implementation reference.
...
merged the runner-as-template / per-slot identity redesign.
...
**Goal.** Wire the config CRUD to the wireframes in `design/runner-mvp-design.pen`. This is the first chunk a non-engineer can interact with.

**Scope note — Runners are top-level in MVP, but the dedicated Runners pages land in C8.5.** C5.5a already moved runners out from under crews and made the same runner shareable across crews; the data model has no notion of "crew-scoped runner" anymore. C3 still does runner CRUD inside Crew Detail (Add Slot + edit drawer) because that's the path the demo flow needs. The standalone Runners list and Runner Detail frames in `design/runner-mvp-design.pen` (`2Oecf`, `ocAFJ`) are built in C8.5 (sibling chunk of C8 signal router).
