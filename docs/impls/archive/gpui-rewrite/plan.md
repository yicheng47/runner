...
Single program document for the Rust-native UI rewrite on the `gpui-nightly` line: strategy, standing decisions, workstreams, and the roadmap. Merged 2026-08-18 from impl 0031 (approach plan) and impl 0046 (main-parity catchup); those numbers remain the citation keys used in the program log (pruned 2026-09-18, in git history). Tracking issue: [runner#307](https://github.com/yicheng47/runner/issues/307).

## Status

**Cutover executed 2026-08-23 (decision 14):** `gpui-nightly` promoted to `main` by tree-swap merge (`8872100`), M6.21 and M6.22 landed on top, and `v0.6.0` tagged on the M6.22 landing (`23548fc`) and **published 2026-08-23 16:54 UTC+8** — the first native release, universal, with the Tauri bridge; `releases/latest` now serves the Sparkle appcast and the bridge manifest. Phases 1–6 and M0–M5 are complete; the program is shipped. M6's remaining items and everything after land on `main` as nightlies (`0.7.0-nightly`) per [m6-consolidation.md](m6-consolidation.md), tracked on GitHub in [#432](https://github.com/yicheng47/runner/issues/432). The rest of this document is the program's record.
...
14. **Cutover pulled forward (2026-08-23, Jason: "we can just do the main branch cutoff; other features or improvements can be pushed after").** With ~15 downloads across every release, the two-week daily-drive criterion and the throwaway-Tauri `bridge-test` rehearsal protect almost nobody, so both are waived: M6.16, M6.19, M6.20, M6.15 and the rest of M6 ride as post-GA updates. What stays mandatory is structural — the native `release.yml` (the branch has none; `main`'s is Tauri-only and the swap deletes it), `ci.yaml` triggering on `main`, the bridge trio with both platform keys, and a `workflow_dispatch` dry run into a draft before the real tag (brief: `briefs/cutover-1-release-workflow.md`). The bridge is proven with the real artifacts instead: on cutover day Jason installs v0.5.2 from the releases page (the Tauri `Runner.app` is no longer on his machine; the nightly is quit first) and lets it hop into native 0.6.0 on the same database before anything is announced; a failed hop costs a manual DMG install, never data. Phase 6's criteria are amended accordingly.
...
5. **M4 — UI rebuild to parity** (new 2026-08-18; inventory and stages refined 2026-08-18): the native UI today is the Phase 3 walking-skeleton look end to end — layout, spacing, colors, chrome, and copy all deviate from the Tauri app, and most product surfaces do not exist at all. M4 is therefore a **rebuild of the UI to `main`'s design**, not a polish pass: nothing currently rendered is presumed to survive as-is. The entry audit is [m4-surface-inventory.md](m4-surface-inventory.md): 75 parity surfaces, 3 functional-but-unstyled, 22 partial, 50 missing. The evidence swaps the old M4.4/M4.5 scopes so lower-complexity entity pages mature the shared list/form/editor primitives and Crew Editor launch path before the mission workspace consumes them. Stages, in order, each a serial mission run daily-driven:
   ...
   - **M4.6 — Settings, command palette, and multi-window** (mission split a–f pinned in the program log (pruned 2026-09-18, in git history)'s 2026-08-20 entry):
...
Runbook additions from Cutover 1 (landed 2026-08-23, `30ea908`, merge `f00324e`): the native `release.yml` exists — `v*` tags and a `dry_run` dispatch build the universal production bundle, the per-release appcast, and (for 0.6.0) the Tauri bridge trio, all onto a **draft**; the bridge signature is verified against the minisign key id the 0.5.x clients embed (`23fee1fa29746d59`), and dry-run drafts are prereleases. Two cautions: dispatch the dry run only for a sha that has a CI run (the CI-green gate polls `ci.yaml` for `$GITHUB_SHA` — after a landing push, never from an unpushed branch), and a retry after a post-build failure needs the existing draft deleted first (`gh release create` refuses a duplicate by design). `ci.yaml` now triggers on `main`; `nightly.yml`'s refs already point at `main`.
