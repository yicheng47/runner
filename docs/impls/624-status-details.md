...
Design signed off by Jason on 2026-09-17: four frames on `design/runner.pen` (`z92Zy9`, `qKf92`, `HiZ5G`, `bqwlq`). Mission 1 (the three UI-only items plus `failed_since`) runs on `feat/624-status-details` with the codex peer crew; brief at [`archive/gpui-rewrite/briefs/624-m1-status-details.md`](./archive/gpui-rewrite/briefs/624-m1-status-details.md). Mission 2 (the working detail) follows once mission 1 has merged.
...
7. **Two missions, one PR each.** Mission 1: interrupted tooltip, wait timers, failure attention, all in `agent_status.rs` and its consumers plus `failed_since`. Mission 2: the working detail, which is the only item touching the adapters and the hook registration. Each stops at a clean working-tree review; Claude lands the PR after its own pass and green CI on both platforms.

## Mission sequence

| Mission | Scope | State |
| --- | --- | --- |
| 1 | `Idle · Last response interrupted` tooltip and `Idle · Interrupted` card line; wait timers in tooltip and card; `failed_since` with sidebar glyph, rollup count and acknowledgement; smoke record | started 2026-09-17 |
| 2 | `WorkDetail` on the observation; Claude `PreCompact`/`PostCompact` registration; Using tools and Compacting context in the three adapters; tooltip and card detail | after mission 1 merges |

## Log

- 2026-09-17 — Hook-capability audit of the five deferred items against the three adapters and the registered hook lists (table in the spec). Four design frames built as copies of the 347 spec rows and signed off. Spec, plan and mission-1 brief committed on `feat/624-status-details`; mission 1 started with the codex peer crew.
