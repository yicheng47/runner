...
- [`459-clear-session-rekey.md`](./459-clear-session-rekey.md) — bug [#459](https://github.com/yicheng47/runner/issues/459) fix plan: a per-spawn claude-code `SessionStart` hook reports the live session id through a watched drop dir, so `/clear` rekeys the row and resume/fork stop restoring the pre-clear conversation.
