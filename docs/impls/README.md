...
- [`437-windows-nightly.md`](./437-windows-nightly.md) — feature [437](../features/437-windows-nightly.md) build plan ([#437](https://github.com/yicheng47/runner/issues/437)): four PR-sized phases from a compile gate (`app_paths`, `ipc`, the `process` lifecycle seam, CI job) through a first `nightly-windows` zip, Job Object lifecycle with both `runner` shims and the named-pipe MCP, to polish. Amends the spec's keymap rule to `Ctrl+Shift+<letter>` and drops the Windows orphan-sweep kill. Written 2026-09-05.
