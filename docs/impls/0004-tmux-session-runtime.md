...
| 8. Resume + status + config reconciliation | ✅ done | next | `SessionRuntime::status()` returns `Option<SessionStatus { alive, exit_code, pid, command }>` parsed from `list-panes -s -F '#{pane_id} #{pane_dead} #{pane_dead_status} #{pane_pid} #{pane_current_command}'`; `Ok(None)` = terminal-unavailable. `TmuxRuntime::reconcile_config()` reads `@runner_config_version` via `show-options -g -v` and `source-file`s the runner config when stale. `parse_pane_status_line` has 5 unit tests for alive/dead/multi-word-command/bad-shape; integration tests cover alive→dead→removed lifecycle and reconcile-no-op-on-fresh-server. Bug fix during integration testing: `has-session`/`kill-session` need `-t` before the target arg — bare argv didn't work even though tmux's interactive parser is more forgiving. |
