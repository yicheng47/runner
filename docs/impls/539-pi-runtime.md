...
The spec's decisions 1–10. For mission 1: the three prompt layers go on `--append-system-prompt <file>` for every spawn and only the lead's goal is a first turn, with the other four runtimes' bodies pinned byte-identical (1); keys are caller-assigned with no capture thread, a missing conversation keeps its key, and the probe reads pi's default session directory only (2); fork is one direct spawn with its own prompt file (3); `--approve` goes on mission slots only, never chats (6); every permission function is empty for pi (7); quiet start is the env var, not `--offline` (8). For mission 2: hooks load through `-e` from app data and nothing is written into `~/.pi` (4); Idle comes only from `agent_settled`, `session_shutdown` clears waits, and pi older than 0.84.4 gets estimated status (5); the rekey drop file reuses `ClaudeSessionKeyWatcher` with no new Rust (10).
