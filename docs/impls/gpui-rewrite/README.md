...
- [m6-consolidation.md](m6-consolidation.md) — M6 plan, pending on top and landed at the bottom: resize smoothness (M6.6), long-lived terminals (M6.8), the `arch.md` rewrite, update hint, sidebar/first-paint fixes, feed text fidelity, the mutex item, real input-state tracking (M6.1) and silent update checks (M6.18) all landed by 2026-08-23; before `v0.6.0`: universal builds (M6.17), descendant sweep (M6.16), pin the UI font to Inter (M6.19), ⌘1–9 tabs (M6.15); post-GA: hook-based session status (M6.2), terminal performance (M6.7), feed append (M6.4), the M6.3 remainder — under the `gpui-nightly`-only landing rule.
