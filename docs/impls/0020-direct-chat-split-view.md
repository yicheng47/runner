...
- The split is a **chat group** — a binding between specific sessions, not a viewport mode. It renders while the open chat is a member; any other chat opens classic single-pane and the group stays intact in the background until a member is opened again. Navigation never mutates the group.
- **Sidebar** reflects the on-screen group: every pane-open chat row gets the selected-row fill (#33353D); the focused pane's row additionally shows a 2px accent bar on its left edge (mirrors the pane focus ring). Clicking a member's row focuses its pane; a non-member row is a plain navigation — unless the group is on screen with an **empty focused pane**, which is the sidebar's explicit "fill this pane" gesture and the one row click that adds a member. *(Originally move-not-copy on every click — dogfooding showed navigation kept mutating the group: moving a visible chat emptied the pane it came from, and opening an unrelated chat got sucked into the layout, evicting a member. Move-not-copy still holds for the assignment op itself: a chat lives in exactly one pane.)*
...
6. **The layout is a per-window chat group, sticky, persisted by the main window.** A module-level store (shared by `RunnerChat` + `Sidebar` via `useSyncExternalStore`) holds the pane tree + focused pane. The group renders only while the open chat is a member — other chats render single-pane over it, one group exists per window (picking a preset from a non-member chat replaces it), and neither route changes nor leaving the chat surface reset it. The main window mirrors it to localStorage (`runner.chat.layout`) so a relaunch restores the same pane grouping — chats come back stopped (the backend kills PTYs on quit) but in their panes, resumable. Persisted as preset + slot assignments + sizes and rebuilt through the preset builders, so a stale payload can't produce an unknown shape; sessions that vanished while the app was closed are swept to empty panes after the first chat-list fetch. Secondary windows stay in-memory — their labels don't survive a relaunch, and localStorage is shared, so persisting theirs would clobber the main window's layout. *(Originally in-memory-only, reset on leaving the surface; revised after dogfooding — losing the grouping on every restart read as a bug.)*
...
- ~~Extract the split machinery out of `RunnerChat`~~ — shipped as `src/components/ChatPaneGroup.tsx`, and further than planned: ONE render path for every arrangement (a single chat is a group of one; non-member chats get an ephemeral single-leaf layout). The earlier split/classic dual path restyled wrappers and re-parented overlays on every mode transition, and that seam is where a string of dogfooding bugs lived. `RunnerChat` keeps orchestration (store, navigation, session lifecycle, keyboard); the group owns chrome, the terminal stack, and geometry.
- ~~Keep terminals mounted across the `metaLoaded` navigation gap~~ — done: the group stays mounted through navigation; archived-row PTY safety is enforced per session by the attach gate. Paired with a backend fix for the root of the "garbled scrollback" dogfooding reports: full-repaint runtimes (claude-code, codex) get their output buffer purged on resize (`SessionManager::resize`), since pre-resize bytes describe a stale grid width and mangle any later snapshot replay — the SIGWINCH repaint rebuilds the buffer at the new width.
