...
- **Landed on `nightly-windows`**: Phase 0 compile gate ([#482](https://github.com/yicheng47/runner/pull/482), `9a213e5`), Phase 1 it opens on the PC ([#483](https://github.com/yicheng47/runner/pull/483), `952767c`), Windows title row fix for [#484](https://github.com/yicheng47/runner/issues/484) ([#485](https://github.com/yicheng47/runner/pull/485), `2a28ada`), PTY burst coalescing for the ConPTY cursor jump ([#486](https://github.com/yicheng47/runner/pull/486), `36f98eb`), Phase 2 agents run ([#487](https://github.com/yicheng47/runner/pull/487), `c946827`).
- **Current nightly**: `Runner-Nightly-0.7.5.20260905.1323-x64.zip` from `2a28ada`, run 33968763863.
- **Next**: Phase 3 as a codex-crew mission landing on `nightly-windows`; then a nightly cut for Jason's PC pass, which is the first real agent acceptance (Phase 1 checklist leftovers, the #484 checks, and Phase 2's list).
...
1. **Phase 3 — polish.** `explorer /select`, icon resource, long-path manifest, ConPTY resize fixtures, promote `Rust / Windows` to required on `nightly-windows`, release notes with the SmartScreen step. The CJK IME pass and DPI drag are PC checks.
2. **PC checklist items not yet reported** — Phase 2 acceptance, in the plan's order: claude direct chat answers and Stop leaves no `node.exe`; codex direct chat likewise; a two-slot crew mission where the lead's `runner signal` reaches the Feed and the reviewer's inbox and Stop leaves no orphans; Runner killed from Task Manager mid-mission leaves no orphans; relaunch shows stale rows demoted with no sweep kills; Settings → Agents shows claude and codex installed; `runner` and `runner.cmd` both exist under `%APPDATA%\com.wycstudios.runner\bin`; a multi-line mission goal reaches the agent as multiple lines. Cursor no longer jumps during a codex redraw (#486). Phase 1 leftovers: database under `%APPDATA%`, Settings pipe name and Ctrl+A/C/X/V, PowerShell pane, Ctrl+C interrupt, Ctrl+[ as ESC, IME, DPI drag; and the #484 checks (drag, double-click, caption buttons, snap flyout, F11, Alt+Tab name, top-edge resize).
