...
- **In flight:** the general `runner` CLI ([spec](./features/648-runner-cli.md), [plan](./impls/648-runner-cli.md)), missions 1 and 2 merged; the gate on a nightly, then mission 3 removes the MCP integration. Mission 4 (the pinned Runner skill row in Settings → Skills and the `runner` command install row) can run while the gate does. The missions-as-containers spec ([`features/562-mission-spawn.md`](./features/562-mission-spawn.md)) is written and waits for 0.12.
...
| [#648](https://github.com/yicheng47/runner/issues/648) general `runner` CLI | P1 | Spec and command set designed; [plan](./impls/648-runner-cli.md) in five missions; mission 1 (the command tree, the caller handle, the backend tools) merged in [#650](https://github.com/yicheng47/runner/pull/650), mission 2 (feed follow, the agent guide, the embedded skill on all five runtimes, three session commands) merged in [#652](https://github.com/yicheng47/runner/pull/652); the four-runtime gate on a nightly comes before mission 3 |
