...
Windows keeps its public `nightly-win` prerelease and baked updater URL. macOS uses the public `nightly` prerelease with a signed Sparkle appcast; stable feeds and `releases/latest` remain separate. The workflow uploads the installer before its signature, verifies release flags and anonymous downloads, then keeps the newest ten installers by stamp with paired signature deletion. A single-platform cut changes only that channel. A failure during publication can leave a partial cut and must be reported incomplete. The first live unified cut and installed upgrades are pending after landing; `/nightly check [both|macos|windows]` verifies an authorized cut without installing it.
