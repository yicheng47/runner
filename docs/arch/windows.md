...
GitHub Copilot CLI and pi are enabled by default on Windows as well as macOS. Runner detects their native binaries or npm `.cmd` shims. Copilot native binaries receive their first turn through `-i`, while batch shims use the existing verified paste fallback; Copilot requires a subscription and npm installations require Node.js 22. Pi receives its system prompt from Runner's per-session file through `--append-system-prompt`, its lead goal behind `--`, and caller-assigned `--session-id` values on fresh and resumed sessions; Runner passes `--approve` only to mission slots to clear pi's project-trust prompt. Pi's bash tool requires Git Bash. Pi reports hook-based status through a Runner-owned Node extension mounted with `-e`, the same reporter on Windows and macOS, while `/new`, `/resume`, and `/fork` rekey the Runner session row through the shared drop watcher. Copilot's caller-assigned session keys and JSONC-preserving folder-trust seed work on both platforms. Copilot, Claude Code and Codex report hook-based status on Windows as on macOS ([#610](../features/archive/610-windows-hook-status.md)); Claude Code runs its status hooks under Git Bash, and Codex and Copilot under PowerShell.

Antigravity CLI is disabled by default on Windows until a JASONPC smoke passes ([#644](../features/644-antigravity-runtime.md) decision 8); it can be enabled in Settings → Agents. Its session-key capture from `--log-file`, `--conversation` resume, folder-trust seed and permission flags have no platform branch, so an enabled agy gets them on Windows too, but its hook status is macOS-only and a Windows session stays on the terminal-activity baseline. The binary path agy's docs give (`%LOCALAPPDATA%\Antigravity\`), its behaviour under ConPTY, and its `cmd /c` hook commands are unverified.

OpenCode is disabled by default on Windows until a JASONPC smoke passes ([#592](../features/592-opencode-runtime.md) decision 10); it can be enabled in Settings → Agents. OpenCode ships a native Windows binary; an npm install puts an `opencode.cmd` shim first on `PATH`, which receives its first turn through the verified paste fallback instead of `--prompt`. Its session-key plugin, the shared drop watcher, `--session` resume, `--session <id> --fork` and `--auto` have no platform branch, and it reports no hook status on either platform. Its database path on Windows (`%USERPROFILE%\.local\share\opencode\opencode.db`, OpenCode's `xdg-basedir` default), the plugin under OpenCode's Windows build, and its rendering under ConPTY are unverified.
...
From the repository root, use `.\make.cmd build` to build the app and CLI sidecar, or `.\make.cmd run` to build and launch. Add `--release` for optimized binaries. The shortcut works in PowerShell and Command Prompt without GNU Make and defaults to 12 build jobs unless `CARGO_BUILD_JOBS` is set.

Development outputs are `target\debug\Runner.exe`, `runner-agent-cli.exe`, `conpty.dll`, and `OpenConsole.exe`; optimized local builds use `target\release`. After Cargo builds, `make.cmd` runs `script/windows/conpty.ps1` to download the pinned, checksum-verified ConPTY package into `target/tools`, verify Microsoft's signatures, and copy the two x64 files beside the executables. Run `.\target\debug\Runner.exe` to launch an existing development build without rebuilding.
...
The `nightly-win` release and tag no longer exist, so a dormant Windows install that missed the hop reinstalls from `nightly`; there was no transition mirror. On macOS, install the first unified DMG over `Runner.app` and delete `Runner Nightly.app` by hand because Sparkle cannot cross bundle identifiers. Both platforms now replace the same app when switching channels; installing a production build by hand switches back to stable.
