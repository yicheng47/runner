...
**Platform target.** macOS on Apple Silicon, and only macOS. An Intel build will use a separate release pipeline when needed. Linux and Windows are out of scope, so no cross-platform fallback paths are maintained and Unix-only mechanisms are used freely.
