...
Claude Code and Codex now have lifecycle hook adapters on macOS. Copilot and TRAE remain on estimated terminal activity and title status, as do Windows agents; Copilot does not emit the braille title prefix, so its estimate uses byte activity and input submission until its hook adapter lands.

The adapter contract is [#347, hook-based agent status](../features/347-hook-based-session-status.md): supported Claude Code and Codex lifecycle adapters own activity, title-spinner classification is removed, and output silence never overrides a healthy hook-driven turn. Missing hooks or a failed bridge need explicit source/availability semantics instead of silently claiming readiness. [#586](../features/586-shell-status-detection.md) separately tries process detection for shell command lifetime before optional semantic shell integration. [#587](../features/archive/587-terminal-provided-titles.md) displays terminal-provided titles independently of status and routing.
