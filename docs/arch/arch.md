...
- **Draft-aware.** If the human is typing in that slot's pane (today a byte-level latch: printable input and pastes set it, Enter/Ctrl-C clear it, with a 10-minute abandonment backstop; M6.1 replaces the latch with an observed input state), the delivery waits in the outbox and the pane shows a "delivery waiting" pill (`router/delivery-blocked`).
