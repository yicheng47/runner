...
A reusable template: handle, display name, runtime, command + args, working dir, system prompt (persona), env, optional model and effort. Known runtimes use the `Runtime` enum in code (`ClaudeCode`, `Codex`, `Trae`, `Copilot`, `Pi`, `Antigravity`, `OpenCode`, `Shell`); SQLite runtime names remain plain strings, and legacy or arbitrary names, including `qoder` rows from before v0.6.7, stay unchanged and readable without a migration. Dispatch parses those names while preserving the existing behavior for unknown runtimes. **Top-level, not nested under a crew.** The same role can be used by many crews simultaneously, and can also be the subject of standalone direct-chat sessions.
...
Settings → Skills reads each runtime's declared user roots. OpenCode contributes `~/.config/opencode/skills` and the roots it shares with Claude Code and the `.agents` runtimes, `~/.claude/skills` and `~/.agents/skills`, as a read/edit catalog with no Runner toggle; a Claude Code toggle does not hide a skill from OpenCode. Antigravity CLI contributes `~/.gemini/antigravity-cli/skills` and `~/.gemini/skills` as a read/edit catalog with no Runner toggle. TRAE CLI contributes only `~/.trae/skills` (not its `.coco` or `.trae-cn` compatibility roots); its pane is a read/edit catalog with no Runner toggle, and its caption points to the skill frontmatter's `disable-model-invocation` switch.

Settings → MCP reads the union of Claude Code, Codex, TRAE CLI, GitHub Copilot CLI, Antigravity CLI, and OpenCode's global server entries directly from their config files, with no Runner-side server store. A runtime dropdown selects a detected, enabled agent; toggles register or unregister the named server for that agent, copying the first registered entry in agent order when turning it on. A server named `runner` is an ordinary user-configured row with the same toggle and editor as every other server. The detail modal shows each agent's native entry and any conflicting definition; its JSON/TOML editor can also translate the change to the other registered agents while preserving their unmodelled keys. Each write changes only the named entry and preserves the rest of the file, including formatting and comments; adding servers and auth flows stay with the agents' own tooling. Reads refresh on entry, Refresh, and after writes; running sessions pick up changes on their next launch. Copilot's registration is `~/.copilot/mcp-config.json` under `mcpServers`, with a `local` entry containing `command`, empty `args`, and `tools: ["*"]`; the file is created when absent. Antigravity CLI's is `~/.gemini/config/mcp_config.json` under `mcpServers`, which agy ships at 0 bytes and Runner reads as `{}`; a copied stdio server is written the way `agy mcp add` writes it, `args` (omitted when empty), `command`, `disabled: false` and `env`, while an HTTP server is refused with a pointer to `agy mcp add` because agy's HTTP entry shape is unprobed. OpenCode's is `~/.config/opencode/opencode.json` under `mcp` (or an existing `opencode.jsonc` when there is no `opencode.json`, the file `opencode mcp add` picks), written the way `opencode mcp add` writes it: `{"type":"local","command":[command, …args],"environment":{…}}` or `{"type":"remote","url":…,"headers":{…}}`, with `environment` and `headers` omitted when empty. The file is JSONC, so the JSON splicer scans an offset-preserving copy with comments and trailing commas blanked and splices the original; every byte outside the rewritten entry survives, the entry itself is replaced whole as `opencode mcp add` replaces it, and a result that does not parse is never written. The 0.11 upgrade step consults the legacy `initializedMcpClients` set once and removes only a `runner` entry with no arguments whose command is this installation's legacy bridge executable under `<app data>/bin`; another installation's entry and a hand edit are logged and left unchanged. A config that cannot be read or parsed is left untouched for that launch and logged as deferred; a second-read race or write failure leaves the step pending for the next launch. See [#555](../features/archive/555-mcp-settings.md).

Preferences persist in `$APPDATA/ui-settings.json`, read by the app at launch; a first launch starts from defaults (resume-on-launch on).
...
Antigravity CLI assigns its own conversation id, and only when the first message is sent, so a fresh spawn carries no key. `session/agy_capture.rs` tails the session's `--log-file` for agy's `Created conversation <uuid>` line and writes that id into `agent_session_key`, guarded by the row's start time and running status; a blank chat keeps tailing until its first message or the end of the session. A resume passes `--conversation <key>` after the role's args. The probe checks `~/.gemini/antigravity-cli/conversations/<key>.db`, a global store, so the cwd does not matter; when it is missing the resume starts fresh with the cold-start first turn and captures a new key, and when agy itself falls back to a new conversation, that conversation's line replaces the key. The log is cleared before every spawn, removed with its row, and swept at startup for rows that no longer exist. Before every spawn, `session/agy_trust.rs` adds the exact cwd to `trustedWorkspaces` in `~/.gemini/antigravity-cli/settings.json`, rewriting only that member so every other byte of agy's file stays as agy wrote it. Accept edits writes `--mode accept-edits`, Bypass `--dangerously-skip-permissions`, and Auto is not offered; the strip covers both flags in every spelling Go's `flag` package accepts (`-x`, `--x`, `-x=v`, `--x=v`).

OpenCode also assigns its own `ses_…` id at the first message, and nothing it stores says which process created a session, so every OpenCode spawn loads a Runner-owned plugin: `<app data>/opencode/runner-session-key.js`, regenerated at startup and named in `OPENCODE_CONFIG_CONTENT`'s `plugin` list. When the role's env or Runner's own environment already sets that variable to a JSON object, Runner appends its plugin to that object's list instead; any other value is left alone and the session reports no key. The plugin runs inside that one OpenCode process and, on every `session.created` without a `parentID`, writes `{"session_id":"ses_…"}` atomically to the row's drop file under `session-keys/`, the path in `RUNNER_OPENCODE_REKEY_PATH`; the shared drop watcher that Claude Code and pi use rekeys the row, and accepts a `ses_…` id only for an OpenCode row. A `--prompt` first turn, a blank chat's first typed message, a fork, a fresh fallback and `/new` all report; a plain resume reports nothing, so its key stands. A resume passes `--session <key>` after the role's args. Before it, Runner checks the key with a read-only `SELECT` in OpenCode's database, resolved like OpenCode does (`OPENCODE_DB`, else `$XDG_DATA_HOME/opencode/opencode.db`, else `~/.local/share/opencode/opencode.db`); when neither WAL side file exists it opens the file `immutable=1`, because a read-only connection cannot create them. A missing row resumes fresh with the persona as the first turn; an unreadable database lets OpenCode decide. Bypass writes `--auto`, Accept edits and Auto are not offered, and the strip covers `--auto`, its hidden aliases `--yolo` and `--dangerously-skip-permissions`, their `=value` forms and `--no-auto`.

Fork creates a new direct-chat row without writing the source row, key, PTY, or conversation file. The new row copies the source's project, role, cwd, runtime, command, model, and effort columns. Claude Code starts the visible TUI directly with `--resume <source> --fork-session --session-id <new>` and a caller-assigned key; Pi uses `--fork <source> --session-id <new>` with the fork row's own refreshed system-prompt file; Codex runs a bounded headless `exec fork` until it has a lineage-validated `thread.started` key, persists that key, marks the temporary row stopped, and starts the visible PTY through the ordinary resume path; OpenCode starts the visible TUI with `--session <source> --fork`, creates the fork itself at startup, and its plugin reports the fork's id like any new session. A direct-spawn, materialization, or resume failure removes both the fork row and its tab. The runtime definition's `native_fork` capability enables Claude Code, Codex, Pi and OpenCode and excludes TRAE, Copilot and Antigravity. An untouched Claude fork is intentionally copy-on-write: manual resume falls back to a fresh chat and launch-time resume reports it unavailable; re-fork the source instead of adding recovery state.
...
`runner help agents` is compiled into the CLI and therefore matches its installed version. It summarizes the command tree, references, output modes, exit codes, identity rule, and mission flow. Starting delegated work requires arming exactly one ID-bound watcher that delivers events, errors and exit to the initiating agent during later work and while idle; merely redirecting to an unread log or waiting only for completion is insufficient. The CLI start command itself remains one-shot, with unchanged ID/JSON output and no new flag. If the host cannot deliver events, the agent must say automatic watching is unavailable and give the foreground follow command. The guide names no host versions, because compiled text cannot follow host releases: a facility qualifies only if its own documentation says each output line and the exit reach an idle agent, with Claude Code's Monitor (`2>&1`, `timeout_ms` at its maximum) as the one worked example. The dated per-host evidence lives in [#686's smoke plan](../tests/archive/686-watch-cli-started-missions.md).
...
Both generated skills require start-and-watch without a separate user request, one watcher per exact returned mission ID, immediate handoffs/questions/failure reporting, and cleanup at lifecycle end. The development skill uses its quoted absolute sidecar for both start and feed. On an expired/broken watch, the agent reports the gap, checks mission state, and re-arms one watcher from the `next_offset` of the last event line it received; only when none arrived does it catch up from offset 0 and skip event IDs it has seen. Host limitations must be reported honestly. The skill states the same qualifying test as the guide and names only Claude Code's Monitor; everything else takes the limitation path unless its documentation says it delivers while idle. These are workflow instructions, not host configuration changes. Updated skills reach real agent homes only through an app build and the normal app-managed installation path.

The three user roots are `~/.claude/skills`, `~/.agents/skills` (shared by Codex, Copilot, pi and OpenCode, which also reads `~/.claude/skills`), and `~/.trae/skills`; Antigravity CLI reads none of them, so it gets no Runner skill. The `runnerSkillEnabled` switch in Settings → General is the truth and defaults on. When on, launch and every runtime-discovery refresh install or refresh the skill in each root read by a detected runtime; the Settings → Agents enable switch is deliberately irrelevant, and a marked folder deleted by hand returns. When off, Runner removes every owned folder from all three roots and does not reinstall it. A root no detected runtime reads is left alone.
...
roles (
  ...
  runtime TEXT NOT NULL,              -- claude-code | codex | trae | copilot | pi | antigravity | opencode | shell (qoder: legacy rows only)
