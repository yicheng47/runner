...
Runtime argv is composed by the adapter in `router/runtime.rs` from the stored runner args, and is not itself stored: the permission mode (`--permission-mode` / codex `--ask-for-approval` + `--sandbox`) follows `MissionPermissionMode` for mission slots, while attended chats strip permission flags without replacement; the adapter adds model and effort flags, codex's `--add-dir` grant for the mission directory, the first-turn body, and — for claude-code — one compact `--settings` JSON that selects Claude Code's alternate-screen renderer (`"tui":"fullscreen"`), installs the `/clear` rekey hook, and, when the effective mode is Bypass, acknowledges Claude Code's bypass consent dialog (`"skipDangerousModePermissionPrompt":true`) so a mission slot never waits on it, unless the runner's own args already pass `--settings`. Runner owns the renderer for the sessions it spawns; `--settings` outranks the user's `~/.claude/settings.json`.
...
The detector shipping in v0.8.9 combines title-spinner classification with byte activity ([#584](../features/archive/584-title-status-detection.md), implemented by #585). Both are heuristics. A CLI's title is presentation, not a guaranteed agent lifecycle contract; foreground process ownership also cannot distinguish thinking from readiness inside an interactive agent.

The current sources and precedence are:

1. **`title`.** `TitleStatus` in `runner-terminal` arms on the first title starting with a braille character (`U+2800`–`U+28FF`), reporting Busy. After that, a nonempty title without the prefix reports Idle. Identical consecutive classifications are deduplicated; blank titles and title resets do not change activity or disarm the detector. `report_declared_status` updates the runtime detector and arms the session's title guard.
...
The next step is [#347, hook-based agent status](../features/347-hook-based-session-status.md): supported Claude Code and Codex lifecycle adapters own activity, title-spinner classification is removed, and output silence never overrides a healthy hook-driven turn. Missing hooks or a failed bridge need explicit source/availability semantics instead of silently claiming readiness. [#586](../features/586-shell-status-detection.md) separately tries process detection for shell command lifetime before optional semantic shell integration. [#587](../features/archive/587-terminal-provided-titles.md) displays terminal-provided titles independently of status and routing.
...
- **Turn-boundary.** Deliveries are spaced by an 80 ms cooldown and a 30 s reconciliation re-nudge covers a nudge that landed while the agent was mid-tool-call; latest-wins absorbs bursts. The planned hook adapter in [#347](../features/347-hook-based-session-status.md) will supply verified turn boundaries; v0.8.9 still uses inferred idle.
