...
The baseline detector infers activity from PTY byte traffic ([#124](../features/archive/13-pty-silence-idle-detection.md)). It is a heuristic: output silence cannot distinguish thinking from readiness inside an interactive agent, and foreground process ownership has the same limitation.
